...
#outlierProb = function(c14,errors,id=NA,oxpath='C:/Users/au209026/Documents/OxCal/bin/OxCalWin.exe',fn=NA)
{
  require(oxcAAR)
  require(readr)
  ...
  oxcalscript <- readr::read_file(fn)
  close(export)
  result_file <- oxcAAR::executeOxcalScript(oxcalscript)
...
#outlierExcluder = function(c14,errors,id=NA,oxpath='C:/Users/au209026/Documents/OxCal/bin/OxCalWin.exe',fn=NA)
{
	require(dplyr)
	require(magrittr)
	...
	if (check&(nrow(tmp)>2))
	{
		...
		df= dplyr::left_join(df,x$df,by='id') %>% select(id,c14=c14.x,errors=errors.x,outlier.prob,exclude)
	} 
